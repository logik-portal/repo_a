"""
Script Name: Find and Replace in Type
Written by: Kieran Hanrahan

Script Version: 1.0.0
Flame Version: 2026

URL: http://github.com/khanrahan/find-replace-in-type

Creation Date: 03.02.25
Update Date: 04.21.25

Description:

    This script will find a specified search string within a Type TimelineFX and
    replace that search term with something else without having to enter the Type
    editor.

    Works on segments or sequences containing Type TimelineFX.  For sequences, it will
    find all segments that have Type TimelineFX and perform the find & replace.

Menus:

    Right-click selected segments in a sequence -> Edit... -> Find and Replace in Type TimelineFX

    Right-click selected sequence or sequences -> Edit... -> Find and Replace in Type TimelineFX
...
TITLE = 'Find and Replace in Type'
VERSION_INFO = (1, 0, 0)
VERSION = '.'.join([str(num) for num in VERSION_INFO])
TITLE_VERSION = f'{TITLE} v{VERSION}'
MESSAGE_PREFIX = '[PYTHON]'

TEMP_SETUP = '/var/tmp/autodesk_flame_find_replace_in_type_temp.type_node'
...
class FlameTypeNodeSetup:
    """Generate an object from a saved type_node file.

    A Flame Type node setup file is an XML with the text stored in <Text> elements as
    a string of unicode ordinals with 1 leading space.

    For example, PYSLATER would be stored as <Text> 80 89 83 76 65 84 69 82</Text>
    """
    ...
    def __init__(self, file=None):
        ...
        Args:
            file: File path to the .type_node setup file.
        """
        self.file = file

        self.type_node_tree = None
        self.load_tree()
        self.type_node_root = None
        self.get_root()

    @staticmethod
    def convert_from_ordinal_string(unicode_int_string):
        """Convert from string of unicode ints to string."""
        return ''.join(chr(int(character)) for character in unicode_int_string.split())

    def convert_from_flame_text(self, string):
        """Convert from Flame-style encoded text to a string."""
        return self.convert_from_ordinal_string(string.lstrip())

    @staticmethod
    def convert_to_ordinal_string(string):
        """Returns Flame style string."""
        return ' '.join(str(ord(character)) for character in list(string))

    def convert_to_flame_text(self, string):
        """Add a leading space to a unicode ordinal string."""
        return ' ' + self.convert_to_ordinal_string(string)

    def load_tree(self):
        """Load the element tree of the type_node file."""
        parser = ETree.XMLParser(encoding='UTF-8')
        try:
            self.type_node_tree = ETree.parse(self.file, parser=parser)
        except Exception as exception:
            raise exception

    def get_root(self):
        """Store the root of the element tree."""
        self.type_node_root = self.type_node_tree.getroot()

    def get_text_elements(self):
        """Get all of the <Text> elements objects in the Type node setup."""
        return list(self.type_node_root.iter('Text'))

    def get_text_strings(self):
        """Get all of the <Text> elements as strings."""
        return [self.convert_from_flame_text(text_elem.text) for text_elem in
                self.get_text_elements()]

    def replace_text(self, find, replace):
        """Find and replace within <Text> elements.

        Args:
            find:   A str to find within the <Text> elements.
            replace:  A str to replace the find with.

        Returns:
            A bool indicating if a replacement was made.
        """
        success = False

        for text_element in self.get_text_elements():
            text_string = self.convert_from_flame_text(text_element.text)
            if find in text_string:
                updated_text_string = text_string.replace(find, replace)
                text_element.text = self.convert_to_flame_text(updated_text_string)
                success = True

        return success

    def write_file(self, output_filepath):
        """Write out the element tree object to file."""
        try:
            self.type_node_tree.write(
                    output_filepath,
                    encoding='UTF-8',
...
class FindReplaceInTypeFX:
    ...
    def __init__(self, selection, **kwargs):
        ...
        self.message(f'Found {len(self.segments)} segments with Type TimelineFX...')
    ...
    @staticmethod
    def save_type_timeline_fx(segment, setup_path):
        """Save out a TTG node setup."""
        for timeline_fx in segment.effects:
            if timeline_fx.type == 'Type':
                timeline_fx.save_setup(setup_path)

    @staticmethod
    def load_type_timeline_fx(segment, setup_path):
    ...
    @staticmethod
    def convert_to_ord_text(string):
    ...
    def filter_segments(self):
        ...
        for item in self.selection:
            ...
            if isinstance(item, flame.PySegment) and not item.hidden.get_value():
    ...
    def process_segment(self, segment, find, replace):
        ...
        self.save_type_timeline_fx(segment, TEMP_SETUP)
        self.remove_timeline_fx(segment, 'Type')
        self.add_timeline_fx(segment, 'Type')
        type_setup_obj = FlameTypeNodeSetup(file=TEMP_SETUP)
        success = type_setup_obj.replace_text(find, replace)
        type_setup_obj.write_file(TEMP_SETUP)
        self.load_type_timeline_fx(segment, TEMP_SETUP)
        return success
    ...
    def main_window(self):
        ...
        def okay_button():
            ...
            for segment in self.segments:
                ...
                change = self.process_segment(segment, self.find, self.replace)
            ...
            if changes_made == 0:
                flame.messages.show_in_dialog(
                    title='No matches!',
                    message=f'"{self.find}" was not found in any Type timelineFX ' +
        ...
        # Tab Order
        self.window.setTabOrder(self.find_line_edit, self.replace_line_edit)
        self.window.setTabOrder(self.replace_line_edit, self.find_line_edit)
...
def find_replace_segments(selection):
    ...
    FindReplaceInTypeFX(selection, target='segments')


def find_replace_sequences(selection):
    """Call the class with a target destination."""
    FindReplaceInTypeFX(selection, target='sequences')
...
def get_media_panel_custom_ui_actions():
    ...
    return [{'name': 'Edit...',
             'actions': [{'name': 'Find and Replace in Type TimelineFX',
