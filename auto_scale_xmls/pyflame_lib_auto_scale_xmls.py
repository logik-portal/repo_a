...
Updates:
    ...
    v3.2.0 09.09.24
        ...
        Added new pyflame functions:
            create_media_panel_folders - Create a folder in the media panel based on the provided folder structure.
            create_file_system_folders - Create a folder in the file system based on the provided folder structure.
    ...
    v3.1.0 09.01.24
        ...
        Added new pyflame functions:
            ...
            print - Print a message to the terminal and Flame message area. Doesn't add --> to message like message_print.
...
class _PyFlameFunctions():
    ...
    @staticmethod
    def create_media_panel_folders(folder_name: str, folder_structure: dict, dest: Union[flame.PyFolder, flame.PyLibrary]) -> None:
        """
        Create Media Panel Folders
        ==========================

        Create folders in the media panel based on the provided folder structure.
        ...
        if not isinstance(folder_name, str):
            raise TypeError(f'PyFlame create_media_panel_folders: Invalid folder_name type: {folder_name}. folder_name must be of type str.')
        if not folder_name.strip():
            raise ValueError(f'PyFlame create_media_panel_folders: folder_name cannot be an empty string or just whitespace.')
        if not isinstance(folder_structure, dict):
            raise TypeError(f'PyFlame create_media_panel_folders: Invalid folder_structure type: {folder_structure}. folder_structure must be of type dict.')
        if not isinstance(dest, (flame.PyFolder, flame.PyLibrary)):
            raise TypeError(f'PyFlame create_media_panel_folders: Invalid dest type: {dest}. dest must be of type flame.PyFolder or flame.PyLibrary.')
        ...
        pyflame.print(
            message=f'Creating Media Panel folders for: {folder_name}',
            new_line=False,
            )
    ...
    @staticmethod
    def create_file_system_folders(folder_name: str, folder_structure: dict, root_folder_path: str) -> None:
        """
        Create File System Folders
        ==========================

        Create folders in the file system based on the provided folder structure.
        ...
        -----
            ...
            root_folder_path (str):
                Path to the root folder where the shot folders will be created.
        """
        ...
        for key, value in folder_structure.items():
            parent_folder = os.path.join(root_folder_path, folder_name)
            if not os.path.isdir(parent_folder):
                try:
                    os.makedirs(parent_folder, exist_ok=True)
                    pyflame.print(
                        message=f'Creating file system folders for: {folder_name}',
                        new_line=False,
                        )
                except OSError as e:
                ...
                pyflame.print(
                    message=f'File system folder: {folder_name} already exists, skipping.',
                    new_line=False,
                    )
    ...
    def copy_to_clipboard(value: str | int) -> None:
        ...
        if isinstance(value, int):
            ...
            raise TypeError(f'PyFlame copy_to_clipboard: Invalid value type: {value}. Value must be of type str or int.')
        ...
        pyflame.message_print(
            message=f'Value copied to clipboard: {value}',
        )

    @staticmethod
    def json_print(json_data, indent=0) -> None:
        """
        JSON Print
        ==========
        ...
        def print_to_terminal(json_data, indent):
                ...
                for key, value in json_data.items():
                    ...
                    if isinstance(value, dict) and not value:
                        ...
                        if isinstance(value, (dict, list)):
                            ...
                            print_to_terminal(value, indent + 4)  # Recursive call
                        else:
            ...
            # If it's a list, iterate through the list items
                ...
                for item in json_data:
                    print_to_terminal(item, indent + 4)  # Recursive call
    ...
    @staticmethod
    def print_list(list_name: str, list_items: list, indent=True, time: int=3, script_name: str=SCRIPT_NAME) -> None:
        """
        Print List
        ===========
        ...
        -----
            ...
            indent (bool):
                Indent message in terminal. Message in Flame message area will not be indented.
                (Default: True)
        ...
        if not isinstance(list_name, str):
            raise TypeError(f'PyFlame print_list: Invalid list_name type: {list_name}. list_name must be of type str.')
        if not isinstance(list_items, list):
            raise TypeError(f'PyFlame print_list: Invalid list_items type: {list_items}. list_items must be of type list.')
        if not all(isinstance(item, str) for item in list_items):
            raise ValueError(f'PyFlame print_list: All elements in list_items list must be strings.')
        if not isinstance(indent, bool):
            raise TypeError(f'PyFlame print_list: Invalid indent type: {indent}. indent must be of type bool.')
        if not isinstance(time, int):
            raise TypeError(f'PyFlame print_list: Invalid time type: {time}. time must be of type int.')
        if not isinstance(script_name, str):
            raise TypeError(f'Pyflame print_list: Invalid script_name type: {script_name}. script_name must be of type str.')
    ...
    @staticmethod
    def print(message: str, indent: bool=False, type=MessageType.INFO, new_line=True, time: int=3, script_name: str=SCRIPT_NAME) -> None:
        """
        Print
        =====
        ...
        -----
            message (str):
                Message to print.
            indent (bool):
                Indent message in terminal. Message will be indented by 4 spaces.
                (Default: False)
            type (MessageType):
                Type of message. See message types below.
                (Default: MessageType.INFO)
        ...
        --------
            To print a message:
            ```
            pyflame.print('This is a message.')
            ```

            To print an indented message:
            ```
            pyflame.print('This is an indented message.', indent=True)
            ```
        """

        # Validate argument types
        if not isinstance(message, str):
            raise TypeError(f'PyFlame print: Invalid message type: {message}. message must be of type str.')
        if not isinstance(indent, bool):
            raise TypeError(f'PyFlame print: Invalid indent type: {indent}. indent must be of type bool.')
        if not isinstance(new_line, bool):
            raise TypeError(f'PyFlame print: Invalid new_line type: {new_line}. new_line must be of type bool.')
        valid_message_types = {MessageType.INFO, MessageType.ERROR, MessageType.WARNING}
        if type not in valid_message_types:
            raise ValueError(f'PyFlame print: Invalid message type: {type}. Accepted types: MessageType.INFO, MessageType.ERROR, MessageType.WARNING.')
        if not isinstance(time, int):
            raise TypeError(f'Pyflame print: Invalid time type: {time}. time must be of type int.')
        if not isinstance(script_name, str):
            raise TypeError(f'Pyflame print: Invalid script_name type: {script_name}. script_name must be of type str.')

        # Print to terminal/shell
        if indent:
            if type == MessageType.INFO:
                print(f'    {message}') # Print message text normally
            elif type == MessageType.ERROR:
                print(f'\033[93m    {message}\033[0m') # Print message text in yellow
            elif type == MessageType.WARNING:
                print(f'\033[91m    {message}\033[0m') # Print message text in red
        else:
            if type == MessageType.INFO:
                print(f'{message}') # Print message text normally
            elif type == MessageType.ERROR:
    ...
    def file_browser(path: str='/opt/Autodesk', title: Optional[str]=None, extension: Optional[List[str]]=None, select_directory: bool=False, multi_selection: bool=False, include_resolution: bool=False, use_flame_browser: bool=True, window_to_hide: Optional[List[QtWidgets.QWidget]]=None) -> Optional[Union[str, list]]:
        ...
        --------
            path (str, list):
                Path to selected file or directory. When Multi Selection is enabled, the file browser will return a list. Otherwise it will return a string.
    ...
    def open_in_finder(path: str) -> None:
        ...
        pyflame.message_print(
            message=f'Opening path in Finder: {path}',
            )
    ...
    def resolve_path_tokens(tokenized_path: str, flame_pyobject=None, date=None) -> str:
        ...
        -----
            tokenized_path (str):
                Path with tokens to be translated.
        ...
        resolved_path = re.sub('<ProjectName>', flame.projects.current_project.name, tokenized_path)
...
class PyFlameConfig:
    ...
    def __init__(self,
        ...
        if not isinstance(config_values, dict):
            raise TypeError("PyFlameConfig: config_values must be a dictionary.")
        if not config_values:
            raise ValueError("PyFlameConfig: config_values dictionary cannot be empty.")
        if not isinstance(config_path, str):
            raise TypeError("PyFlameConfig: config_path must be a string.")
    ...
    def load_config(self) -> None:
        ...
        print('Script configuration loaded.\n')

    def save_config(self, config_values: Dict[str, Any]=None, config_path=None) -> None:
        """
        Save Config
        ===========
        ...
        print('Saving script configuration...\n')
        ...
        print('Script configuration saved.\n')
...
class PyFlameButtonGroup(QtWidgets.QButtonGroup):
    ...
    def __init__(self: 'PyFlameButtonGroup',
        ...
        if not isinstance(set_exclusive, bool):
            raise TypeError(f'PyFlameButtonGroup: Invalid set_exclusive argument: {set_exclusive}. Must be of type bool.')
...
class PyFlameLabel(QtWidgets.QLabel):
    ...
    def __init__(self: 'PyFlameLabel',
        ...
        if not isinstance(underline_color, tuple) or len(underline_color) != 4:
...
class PyFlameTreeWidget(QtWidgets.QTreeWidget):
    ...
    def __init__(self: 'PyFlameTreeWidget',
        ...
        elif not isinstance(font_family, str):
...
class PyFlamePresetManager():
    ...
    def new_preset(self) -> None:
        ...
        if new_preset_name:
            self.message_print(
                message=f'New preset created: {new_preset_name}',
                )
        else:
            self.message_print(
                message='New preset creation cancelled.',
                )
    ...
    def edit_preset(self) -> None:
        ...
        self.message_print(
            message='Editing selected preset...',
            )
        ...
        if preset_name:
            self.message_print(
                message=f'Edited preset: {preset_name}',
                )
        else:
            self.message_print(
    ...
    def set_as_default_preset(self) -> None:
        ...
        self.message_print(
            message='Updating default preset...',
            )
        ...
        if self.current_preset_menu_pushbutton.text():
            ...
            self.message_print(
                message=f'Default preset set to: {self.current_preset_menu_pushbutton.text()}',
                )
        else:
            self.message_print(
                message='No preset selected to set as Default Preset.',
                )
    ...
    def set_preset_to_current_project(self) -> None:
        ...
        self.message_print(
            message='Assigning preset to current project...',
            )
        ...
        if self.current_preset_menu_pushbutton.text():
            ...
            self.message_print(
                message=f'Preset set for this project: {preset_name_text}',
                )
        else:
            self.message_print(
                message='No preset selected to set as Project Preset.',
                )
    ...
    def remove_preset_from_project(self) -> None:
        ...
        self.message_print(
            message=f'Removing preset from project: {preset_name}',
            )
        ...
        if os.path.isfile(project_preset_path):
            ...
            self.message_print(
                message=f'Preset removed from project: {preset_name}',
                )
        else:
            self.message_print(
                message=f'No project preset found for current project.',
                )
    ...
    def create_or_edit_preset(self, preset_path: str=None, preset_config: PyFlameConfig=None) -> None:
        ...
        if new_preset.settings:
            ...
            if not isinstance(new_preset.settings, dict):
                raise TypeError(f'PyFlamePresetManager: new_preset must be a dictionary.')
    ...
    def update_project_presets(self, old_preset_name: str, new_preset_name: str):
        ...
        self.message_print(
            message=f'Updating project presets to new preset name: {new_preset_name}',
            )
    ...
    def get_project_preset_name_json(self, project_preset_path: str) -> str:
        ...
        # # Extract the preset name from the JSON structure
        # preset_name = data.get("settings", {}).get(self.preset_settings_name, {}).get("preset_name", None)
    ...
    def update_ui(self) -> None:
        ...
        self.message_print(
            message='Updating Preset Manager UI...',
            )
        ...
        self.message_print(
            message='Preset Manager UI Updated.',
            )
