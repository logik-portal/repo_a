...
def get_result_camera():
    ...
    if result_cam_name != 'DefaultCam':
        result_camera_num = result_camera_num
    ...
    try:
        ...
        PyFlameMessageWindow(
            message='Action result camera should be set to camera other than default camera.',
...
def save_action_node():
    ...
    # Save Action node
    temp_folder = f'{SCRIPT_PATH}/temp_action'
...
def create_projector_projection(selection):
    ...
    pyflame.print_title(f'{SCRIPT_NAME} - Projector Projection {SCRIPT_VERSION}')
    ...
    pyflame.print('Projector Projection Created', text_color=TextColor.GREEN)
...
def create_diffuse_projection(selection):
    ...
    pyflame.print_title(f'{SCRIPT_NAME} - Diffuse Projection {SCRIPT_VERSION}')
